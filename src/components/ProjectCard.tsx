...
function ProjectCard({ project, onClick, onDelete }: ProjectCardProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow p-4 cursor-pointer group">
      <div className="flex items-start justify-between" onClick={onClick}>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-500 mt-1">{formatMonthDay(project.start_date)} ~ {formatMonthDay(project.end_date)}</p>
          ...
          <div className="flex gap-2 mt-2 text-xs">
            <span className="text-gray-400">{totalDays} 天</span>
            ...
            {!isOverdue && !isEnding && (
              <span className="text-gray-400">剩 {remaining} 天</span>
            )}
          </div>
        </div>
